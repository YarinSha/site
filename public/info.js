...
var firebaseConfig={apiKey:"AIzaSyAaVVlIVaomAuhJxt75hkLrxid6kt1dXVM",authDomain:"amit-6b7b6.firebaseapp.com",projectId:"amit-6b7b6",storageBucket:"amit-6b7b6.appspot.com",messagingSenderId:"207665282730",appId:"1:207665282730:web:e46ea2120b5e23f5a05ad3",measurementId:"G-RK412JV7N4"};firebase.initializeApp(firebaseConfig),firebase.analytics();var storage=firebase.storage(),storageRef=storage.ref(),db=firebase.firestore(),database=firebase.database();const pageTitle=document.getElementById("pageTitle"),title=document.getElementById("title"),tableUpcoming=document.getElementById("tableUpcoming"),tableDone=document.getElementById("tableDone"),tableFinished=document.getElementById("tableFinished"),name=sessionStorage.getItem("NAME"),type=sessionStorage.getItem("TYPE"),of=sessionStorage.getItem("OF"),num=parseInt(sessionStorage.getItem("NUM")),className=sessionStorage.getItem("CLASS");pageTitle.innerHTML=of,title.innerHTML=of;var email="";db.collection("users").where("name","==",of).get().then(function(e){e.forEach(function(e){email=e.id})}).catch(function(e){console.log("Error getting documents: ",e)});var btn=document.createElement("button");function newMeeting(e,t,n,a,i,o){var s;2==tableUpcoming.rows.length&&n.child("Meetings/Upcoming/"+tableUpcoming.rows[1].cells[0].innerHTML.replace(" - ","&")+".txt").getDownloadURL().then(function(e){var t=new XMLHttpRequest;t.responseType="blob",t.onload=function(e){t.response.text().then(e=>{var t=e.split("&&")[2],n=e.split("&&")[3];s=new Date(t.split("/")[2]+"-"+t.split("/")[1]+"-"+t.split("/")[0]+"T"+n)})},t.open("GET",e),t.send()}).catch(function(e){console.log(e)});var l=document.createElement("h4");l.innerHTML="זמן הפגישה:",document.body.appendChild(l);var c=document.createElement("input");c.type="datetime-local",document.body.appendChild(c),document.body.appendChild(document.createElement("br")),document.body.appendChild(document.createElement("br"));var r=document.createElement("button");r.innerHTML="קביעה",r.onclick=function(){if(c.value)if(2==tableUpcoming.rows.length){var m=new Date(c.value),d=Math.abs(s-m)/6e4;if(console.log(d),d<20)alert("זמן זה תפוס, אנא בחר/י זמן אחר");else{(g=document.createElement("h4")).innerHTML="יוצר את הפגישה... נא לא לצאת מהמסך!",document.body.appendChild(g);var p=e+"&&"+t+"&&"+c.value.split("T")[0].split("-")[2]+"/"+c.value.split("T")[0].split("-")[1]+"/"+c.value.split("T")[0].split("-")[0]+"&&"+c.value.split("T")[1]+"&&&&&&",u=new Blob([p],{type:"text/plain;charset=utf-8"});n.child("Meetings/Upcoming/"+e+"&"+t+".txt").put(u).then(function(t){o.includes("@")?n.child("Emails/"+o+".txt").put(u).then(function(t){updateMeetings(i,e,l,c,r,a,g)}):updateMeetings(i,e,l,c,r,a,g)})}}else{var g;(g=document.createElement("h4")).innerHTML="יוצר את הפגישה... נא לא לצאת מהמסך!",document.body.appendChild(g);p=e+"&&"+t+"&&"+c.value.split("T")[0].split("-")[2]+"/"+c.value.split("T")[0].split("-")[1]+"/"+c.value.split("T")[0].split("-")[0]+"&&"+c.value.split("T")[1]+"&&&&&&",u=new Blob([p],{type:"text/plain;charset=utf-8"});n.child("Meetings/Upcoming/"+e+"&"+t+".txt").put(u).then(function(t){o.includes("@")?n.child("Emails/"+o+".txt").put(u).then(function(t){updateMeetings(i,e,l,c,r,a,g)}):updateMeetings(i,e,l,c,r,a,g)})}else alert("לא בחרת זמן לפגישה!")},document.body.appendChild(r)}function updateMeetings(e,t,n,a,i,o,s){var l=database.ref(e+"/"+t);l.once("value").then(e=>{l.set((parseInt(e.val())+1).toString());var t=database.ref("מורים/"+name);t.once("value").then(e=>{t.set((parseInt(e.val())+1).toString()),n.innerHTML="הפגישה נקבעה בהצלחה!",a.style.display="none",i.style.display="none",o.style.display="none",s.style.display="none"})})}btn.innerHTML="קביעת פגישה",btn.onclick=function(){newMeeting(of,name,storageRef,btn,className,email)},storageRef.child("Meetings/Upcoming").listAll().then(function(e){e.items.forEach(function(e){if(e.name.includes(of)){txtName=document.createTextNode(e.name.split(".")[0].replace("&"," - "));var t=tableUpcoming.insertRow();t.insertCell().appendChild(txtName),t.onclick=function(){if(sessionStorage.setItem("MEETING",e.name),sessionStorage.setItem("U/D/F","U"),3==tableUpcoming.rows.length){var n=t.rowIndex;1==n&&sessionStorage.setItem("otherMeet",tableUpcoming.rows[2].cells[0].innerHTML.replace(" - ","&")+".txt"),2==n&&sessionStorage.setItem("otherMeet",tableUpcoming.rows[1].cells[0].innerHTML.replace(" - ","&")+".txt")}location.href="meeting.html"},e.name.includes(name)&&(btn.style.display="none")}}),storageRef.child("Meetings/Done").listAll().then(function(e){e.items.forEach(function(e){if(e.name.includes(of)){txtName=document.createTextNode(e.name.split(".")[0].replace("&"," - "));var t=tableDone.insertRow();t.insertCell().appendChild(txtName),t.onclick=function(){sessionStorage.setItem("MEETING",e.name),sessionStorage.setItem("U/D/F","D"),location.href="meeting.html"},e.name.includes(name)&&(btn.style.display="none")}}),storageRef.child("Meetings/Finished").listAll().then(function(e){e.items.forEach(function(e){if(e.name.includes(of)){txtName=document.createTextNode(e.name.split(".")[0].replace("&"," - "));var t=tableFinished.insertRow();t.insertCell().appendChild(txtName),t.onclick=function(){sessionStorage.setItem("MEETING",e.name),sessionStorage.setItem("U/D/F","F"),location.href="meeting.html"},e.name.includes(name)&&(btn.style.display="none")}}),"מורים"!==className&&num<2&&document.body.appendChild(btn)}).catch(function(e){console.log(e)})}).catch(function(e){console.log(e)})}).catch(function(e){console.log(e)}),window.addEventListener("storage",function(e){e.storageArea===sessionStorage&&(alert("האתר זיהה ניסיון חשוד לשינוי פרטים!"),sessionStorage.clear(),location.href="index.html")});
