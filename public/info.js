...
var firebaseConfig={apiKey:"AIzaSyAaVVlIVaomAuhJxt75hkLrxid6kt1dXVM",authDomain:"amit-6b7b6.firebaseapp.com",projectId:"amit-6b7b6",storageBucket:"amit-6b7b6.appspot.com",messagingSenderId:"207665282730",appId:"1:207665282730:web:e46ea2120b5e23f5a05ad3",measurementId:"G-RK412JV7N4"};firebase.initializeApp(firebaseConfig),firebase.analytics();var storage=firebase.storage(),storageRef=storage.ref(),db=firebase.firestore();const pageTitle=document.getElementById("pageTitle"),title=document.getElementById("title"),tableUpcoming=document.getElementById("tableUpcoming"),tableDone=document.getElementById("tableDone"),tableFinished=document.getElementById("tableFinished"),name=sessionStorage.getItem("NAME"),type=sessionStorage.getItem("TYPE"),of=sessionStorage.getItem("OF"),num=parseInt(sessionStorage.getItem("NUM")),className=sessionStorage.getItem("CLASS");pageTitle.innerHTML=of,title.innerHTML=of;var email="";if(db.collection("users").where("name","==",of).get().then(function(e){e.forEach(function(e){console.log(e.id," => ",e.data()),email=e.id})}).catch(function(e){console.log("Error getting documents: ",e)}),"teacher"===type&&num<2){var btn=document.createElement("button");btn.innerHTML="קביעת פגישה",btn.onclick=function(){newMeeting(of,name,storageRef,btn,className,email)},setTimeout(()=>{document.body.appendChild(btn)},2e3)}function newMeeting(e,t,n,o,i,a){var l=document.createElement("h4");l.innerHTML="זמן הפגישה:",document.body.appendChild(l);var s=document.createElement("input");s.type="datetime-local",document.body.appendChild(s),document.body.appendChild(document.createElement("br")),document.body.appendChild(document.createElement("br"));var c=document.createElement("button");c.innerHTML="קביעה",c.onclick=function(){if(s.value){var r=document.createElement("h4");r.innerHTML="יוצר את הפגישה... נא לא לצאת מהמסך!",document.body.appendChild(r);var m=e+"&&"+t+"&&"+s.value.split("T")[0].split("-")[2]+"/"+s.value.split("T")[0].split("-")[1]+"/"+s.value.split("T")[0].split("-")[0]+"&&"+s.value.split("T")[1]+"&&&&&&",d=new Blob([m],{type:"text/plain;charset=utf-8"});n.child("Meetings/Upcoming/"+e+"&"+t+".txt").put(d).then(function(e){n.child("Emails/"+a+".txt").put(d).then(function(e){updateMeetings(n,i,l,s,c,o,r)})})}else alert("לא בחרת זמן לפגישה!")},document.body.appendChild(c)}function updateMeetings(e,t,n,o,i,a,l){var s=0;e.child("Classes").listAll().then(function(c){c.items.forEach(function(c){if(c.name.includes("Teachers")||c.name.includes(t)){var r="";c.getDownloadURL().then(function(t){var m=new XMLHttpRequest;m.responseType="blob",m.onload=function(t){m.response.text().then(t=>{t.split("&&").forEach(function(m){if(m.includes("==")){const u=m.split("==")[0];var d=0;e.child("Meetings").listAll().then(function(m){m.prefixes.forEach(function(f){f.listAll().then(function(p){if(p.items.forEach(function(e){e.name.includes(u)&&d++}),f.name===m.prefixes[2].name&&(r=r.concat(u+"=="+d+"&&")).split("&&").length===t.split("&&").length){var h=new Blob([r],{type:"text/plain;charset=utf-8"});e.child(c.fullPath).put(h).then(function(e){2==++s&&(n.innerHTML="הפגישה נקבעה בהצלחה!",o.style.display="none",i.style.display="none",a.style.display="none",l.style.display="none")})}}).catch(function(e){console.log(e)})})}).catch(function(e){console.log(e)})}})})},m.open("GET",t),m.send()}).catch(function(e){console.log(e)})}})}).catch(function(e){console.log(e)})}storageRef.child("Meetings/Upcoming").listAll().then(function(e){e.prefixes.forEach(function(e){}),e.items.forEach(function(e){if(e.name.includes(of)){txtName=document.createTextNode(e.name.split(".")[0].replace("&"," - "));var t=tableUpcoming.insertRow();t.insertCell().appendChild(txtName),t.onclick=function(){sessionStorage.setItem("MEETING",e.name),sessionStorage.setItem("U/D/F","U"),location.href="meeting.html"},e.name.includes(name)&&(btn.style.display="none")}})}).catch(function(e){console.log(e)}),storageRef.child("Meetings/Done").listAll().then(function(e){e.prefixes.forEach(function(e){}),e.items.forEach(function(e){if(e.name.includes(of)){txtName=document.createTextNode(e.name.split(".")[0].replace("&"," - "));var t=tableDone.insertRow();t.insertCell().appendChild(txtName),t.onclick=function(){sessionStorage.setItem("MEETING",e.name),sessionStorage.setItem("U/D/F","D"),location.href="meeting.html"},e.name.includes(name)&&(btn.style.display="none")}})}).catch(function(e){console.log(e)}),storageRef.child("Meetings/Finished").listAll().then(function(e){e.prefixes.forEach(function(e){}),e.items.forEach(function(e){if(e.name.includes(of)){txtName=document.createTextNode(e.name.split(".")[0].replace("&"," - "));var t=tableFinished.insertRow();t.insertCell().appendChild(txtName),t.onclick=function(){sessionStorage.setItem("MEETING",e.name),sessionStorage.setItem("U/D/F","F"),location.href="meeting.html"},e.name.includes(name)&&(btn.style.display="none")}})}).catch(function(e){console.log(e)}),window.addEventListener("storage",function(e){e.storageArea===sessionStorage&&(alert("האתר זיהה ניסיון חשוד לשינוי פרטים!"),sessionStorage.clear(),location.href="index.html")});
