...
var firebaseConfig={apiKey:"AIzaSyAaVVlIVaomAuhJxt75hkLrxid6kt1dXVM",authDomain:"amit-6b7b6.firebaseapp.com",projectId:"amit-6b7b6",storageBucket:"amit-6b7b6.appspot.com",messagingSenderId:"207665282730",appId:"1:207665282730:web:e46ea2120b5e23f5a05ad3",measurementId:"G-RK412JV7N4"};firebase.initializeApp(firebaseConfig),firebase.analytics();var database=firebase.database();const pageTitle=document.getElementById("pageTitle"),title=document.getElementById("title"),table=document.getElementById("table"),name=sessionStorage.getItem("NAME"),type=sessionStorage.getItem("TYPE"),className=sessionStorage.getItem("CLASS");pageTitle.innerHTML=className,title.innerHTML=className,database.ref(className).once("value").then(e=>{e.forEach(e=>{var a=e.key,t=e.val();let s=document.createTextNode(a);if("מורים"===className)var n=document.createTextNode(t);else n=document.createTextNode(t+"/2");var o=table.insertRow();o.insertCell().appendChild(s),o.insertCell().appendChild(n),"2/2"===n.nodeValue&&(o.style.color="DarkSlateGray"),o.onclick=function(){sessionStorage.setItem("OF",s.nodeValue),sessionStorage.setItem("NUM",n.nodeValue.split("/")[0]),location.href="info.html"}})}),window.addEventListener("storage",function(e){e.storageArea===sessionStorage&&(alert("האתר זיהה ניסיון חשוד לשינוי פרטים!"),sessionStorage.clear(),location.href="index.html")});
