...
var firebaseConfig={apiKey:"AIzaSyAaVVlIVaomAuhJxt75hkLrxid6kt1dXVM",authDomain:"amit-6b7b6.firebaseapp.com",projectId:"amit-6b7b6",storageBucket:"amit-6b7b6.appspot.com",messagingSenderId:"207665282730",appId:"1:207665282730:web:e46ea2120b5e23f5a05ad3",measurementId:"G-RK412JV7N4"};firebase.initializeApp(firebaseConfig),firebase.analytics();var storage=firebase.storage(),storageRef=storage.ref();const pageTitle=document.getElementById("pageTitle"),title=document.getElementById("title"),table=document.getElementById("table"),name=sessionStorage.getItem("NAME"),type=sessionStorage.getItem("TYPE"),className=sessionStorage.getItem("CLASS");function downloadFile(e,t){e.getDownloadURL().then(function(e){var a=new XMLHttpRequest;a.responseType="blob",a.onload=function(e){a.response.text().then(e=>{for(i=0;i<e.split("&&").length-1;i++){let n=document.createTextNode(e.split("&&")[i].split("==")[0]);if("מורים"===className)var a=document.createTextNode(e.split("&&")[i].split("==")[1]);else a=document.createTextNode(e.split("&&")[i].split("==")[1]+"/2");var s=t.insertRow();s.insertCell().appendChild(n),s.insertCell().appendChild(a),"2/2"===a.nodeValue&&(s.style.color="DarkSlateGray"),s.onclick=function(){sessionStorage.setItem("OF",n.nodeValue),sessionStorage.setItem("NUM",a.nodeValue.split("/")[0]),location.href="info.html"}}})},a.open("GET",e),a.send()}).catch(function(e){console.log(e)})}pageTitle.innerHTML=className,title.innerHTML=className,storageRef.child("Classes").listAll().then(function(e){e.prefixes.forEach(function(e){}),e.items.forEach(function(e){(e.name.includes(className)||"מורים"===className&&e.name.includes("Teachers"))&&downloadFile(e,table)})}).catch(function(e){console.log(e)}),window.addEventListener("storage",function(e){e.storageArea===sessionStorage&&(alert("האתר זיהה ניסיון חשוד לשינוי פרטים!"),sessionStorage.clear(),location.href="index.html")});
