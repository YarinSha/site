...
var firebaseConfig={apiKey:"AIzaSyAaVVlIVaomAuhJxt75hkLrxid6kt1dXVM",authDomain:"amit-6b7b6.firebaseapp.com",projectId:"amit-6b7b6",storageBucket:"amit-6b7b6.appspot.com",messagingSenderId:"207665282730",appId:"1:207665282730:web:e46ea2120b5e23f5a05ad3",measurementId:"G-RK412JV7N4"};firebase.initializeApp(firebaseConfig),firebase.analytics();const userEmail=document.getElementById("userEmail"),userPassword=document.getElementById("userPassword"),btnNext=document.getElementById("btnNext"),btnForget=document.getElementById("btnForget"),inputTxt=document.getElementById("inputTxt"),input=document.getElementById("input"),btnSub=document.getElementById("btnSub"),loadTxt=document.getElementById("loadTxt");inputTxt.style.display="none",input.style.display="none",btnSub.style.display="none",btnNext.addEventListener("click",e=>{inputTxt.style.display="none",input.style.display="none",btnSub.style.display="none",loadTxt.innerHTML="מתחבר...";const t=userEmail.value,n=userPassword.value;firebase.auth().signInWithEmailAndPassword(t,n).then(e=>{sessionStorage.setItem("EMAIL",e.user.email),location.href="menu.html"}).catch(e=>{loadTxt.style.display="none";var t=e.message;alert(t)})}),btnForget.onclick=function(){inputTxt.innerHTML="מייל לשחזור:",inputTxt.style.display="block",input.style.display="block",btnSub.style.display="block",btnSub.onclick=function(){var e=input.value;firebase.auth().sendPasswordResetEmail(e).then(function(){inputTxt.innerHTML="נשלח מייל לשחזור סיסמה",input.style.display="none",btnSub.style.display="none"}).catch(function(e){alert(e.message)})}},window.addEventListener("storage",function(e){e.storageArea===sessionStorage&&(alert("האתר זיהה ניסיון חשוד לשינוי פרטים!"),sessionStorage.clear(),location.href="index.html")});
