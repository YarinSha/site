...
var firebaseConfig={apiKey:"AIzaSyAaVVlIVaomAuhJxt75hkLrxid6kt1dXVM",authDomain:"amit-6b7b6.firebaseapp.com",projectId:"amit-6b7b6",storageBucket:"amit-6b7b6.appspot.com",messagingSenderId:"207665282730",appId:"1:207665282730:web:e46ea2120b5e23f5a05ad3",measurementId:"G-RK412JV7N4"};firebase.initializeApp(firebaseConfig),firebase.analytics();var storage=firebase.storage(),storageRef=storage.ref();const tableUpcoming=document.getElementById("tableUpcoming"),tableDone=document.getElementById("tableDone"),tableFinished=document.getElementById("tableFinished"),name=sessionStorage.getItem("NAME"),type=sessionStorage.getItem("TYPE");storageRef.child("Meetings/Upcoming").listAll().then(function(e){e.prefixes.forEach(function(e){}),e.items.forEach(function(e){if(e.name.includes(name)||"admin"===type){txtName=document.createTextNode(e.name.split(".")[0].replace("&"," - "));var t=tableUpcoming.insertRow();t.insertCell().appendChild(txtName),t.onclick=function(){sessionStorage.setItem("MEETING",e.name),sessionStorage.setItem("U/D/F","U"),location.href="meeting.html"}}})}).catch(function(e){console.log(e)}),storageRef.child("Meetings/Done").listAll().then(function(e){e.prefixes.forEach(function(e){}),e.items.forEach(function(e){if(e.name.includes(name)||"admin"===type){txtName=document.createTextNode(e.name.split(".")[0].replace("&"," - "));var t=tableDone.insertRow();t.insertCell().appendChild(txtName),t.onclick=function(){sessionStorage.setItem("MEETING",e.name),sessionStorage.setItem("U/D/F","D"),location.href="meeting.html"}}})}).catch(function(e){console.log(e)}),storageRef.child("Meetings/Finished").listAll().then(function(e){e.prefixes.forEach(function(e){}),e.items.forEach(function(e){if(e.name.includes(name)||"admin"===type){txtName=document.createTextNode(e.name.split(".")[0].replace("&"," - "));var t=tableFinished.insertRow();t.insertCell().appendChild(txtName),t.onclick=function(){sessionStorage.setItem("MEETING",e.name),sessionStorage.setItem("U/D/F","F"),location.href="meeting.html"}}})}).catch(function(e){console.log(e)}),window.addEventListener("storage",function(e){e.storageArea===sessionStorage&&(alert("האתר זיהה ניסיון חשוד לשינוי פרטים!"),sessionStorage.clear(),location.href="index.html")});
